...
# sh mrs_to_bids.sh sub-NNNNN ses-NN STUDY_CODE ScanID DATE ACC_series## Thal_series## PWM_series## 
# arguments MUST be provided in order, the same info should be listed in each row of the subject list for running a batch
# DATE should be written in format YYYY-MM-DD
...
# Update directory permissions to allow full access to mrs data for users with access to the BIDS directory
chmod -R 777 $BIDS/${1}/${2}/mrs

# Copy P-file to mrs folder and rename
cp -Rv $MRS/SPECT-EXAM${4}-${5}/SERIES*${6}/*.7 $BIDS/${1}/${2}/mrs/${1}_${2}_slaser_acc.7
